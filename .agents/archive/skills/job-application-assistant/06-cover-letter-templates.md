...
- Use local date conventions consistently
- Use "Dear Hiring Manager" only when no named contact is available
- Prefer a direct professional closing such as "Kind regards,"

### Non-English Cover Letters
- Same template structure, but write content in another language only if the posting explicitly requires it
- Adjust date format and closing to the relevant local convention
...
- [ ] Date is current
- [ ] Fits on one page
- [ ] Language is Australian English unless the posting explicitly requires otherwise
