...
The active job portal CLIs (`linkedin-search` and `freehire-search`) are written in TypeScript and run with Bun. The archived Danish portal examples use the same tooling if you ever inspect or adapt them.
...
By default, MiKTeX installs missing packages on demand but pops up a GUI prompt for each one — which blocks non-interactive terminals (including Codex's Bash tool). Turn that into a silent auto-install instead:
...
You can generate an equivalent search skill for an Australian job board with `/add-portal` — it scaffolds the same CLI structure for any public portal and test-runs a live query before registering. See the "Job search tools" section in the README.
