...
name: scrape
description: >
  Finds Australian job postings matching your profile via installed portal-search CLIs
  (LinkedIn, local job boards, and any skills added with /add-portal) plus targeted
  WebSearch fallback. Deduplicates across runs. Triggers on: job scrape, find jobs,
...
This skill searches Australian job sources using the **installed portal-search CLIs**
in `.agents/skills/` plus WebSearch fallback, using queries from your profile. It
deduplicates against previously seen jobs and the application tracker, and presents
...
Australian fallback priorities are SEEK, LinkedIn Australia, Workforce Australia,
APS Jobs, company career pages, and common ATS hosts such as Greenhouse, Lever,
Workday, SmartRecruiters, and Ashby. Target the configured geographic area and
...
10. **Australia-first defaults.** Do not use the archived Danish portal CLI examples unless the user explicitly asks for Denmark and they have first been restored under `.agents/skills/`. Prefer the country-agnostic LinkedIn and freehire tools plus web searches for Australian boards.
