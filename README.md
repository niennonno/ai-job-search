...
The repo also includes **`freehire-search`** — a tech-focused public REST API search skill for software, data, engineering, DevOps, and remote roles across multiple markets. It can be useful for AI/product-adjacent roles, but do not rely on it for broad non-technical Australian product coverage. See `.agents/skills/freehire-search/SKILL.md`.

The Danish portal CLIs are retained in `.agents/archive/skills/` only as legacy examples for building custom portal integrations. To add an Australian board with a dedicated CLI, run:
