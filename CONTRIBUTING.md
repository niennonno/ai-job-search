...
**This repo is a universal template.** People fork it and adapt it to their own market, language, and profile. Upstream stays market-agnostic, person-agnostic, and Claude Code-native; this fork keeps Codex entrypoints as a local compatibility layer. The corollary: a contribution is judged by fit to this rule first, execution quality second. Well-built but off-policy still gets declined (kindly, with reasons).
...
Claude Code is upstream's reference runtime: it is what the maintainer runs daily and what every methodology change is verified on. In this fork, Codex entrypoints are maintained as thin routers over the canonical `.claude/` specs, not as a second independent workflow source. When upstream adds or changes a command, update the matching `.codex/skills/*` entrypoint instead of copying the full spec.
